...
def laptop(request,data=None):
    ...
    elif data=='asus'or data=='hp' or data=='mackbook':
...
def topwear(request,data=None):
    ...
    if data==None:
        topwears=Product.objects.filter(category='TW')
    elif data=='wrong'or data=='Lee' or data=='jeep':
...
def bottomwear(request,data=None):
    ...
    if data==None:
        bottomwears=Product.objects.filter(category='BT')
    elif data=='Lee'or data=='' or data=='':
        bottomwears=Product.objects.filter(category='BT').filter(brand=data)
    elif data=='bellow':
        bottomwears= Product.objects.filter(category='BT').filter(discounted_price__lt=600)
    elif data=='above':
        bottomwears= Product.objects.filter(category='BT').filter(discounted_price__gt=600)
    if request.user.is_authenticated:
        totalitem=len(Cart.objects.filter(user=request.user))
